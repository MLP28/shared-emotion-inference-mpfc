...
% Computes the concordance of mapname1 and mapname2.
    ...
    %%%%%%%%%%%%%%%%%% EDIT %%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    [x1,y1] = find(isnan(map1));
    [x2,y2] = find(isnan(map2));
    for id = 1 : size(x1,1)
        trmsk(x1(id),y1(id)) = 0; 
...
% correlations over the whole volume.
    ...
    covv(:,1) = RSMs(1).RSM(trmsk); %mat ligne des valeurs de RSM(1) filtrées par trmsk (4560*1 double)
    for i = 2 : numel(RSMs)
        ...
        [x,y] = find(isnan(RSMs(i).RSM));
        for id = 1 : size(x,1)
            if trmsk(x(id),y(id)) ~= 0
                trmsk(x(id),y(id)) = nan;
            end
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        covv(:,end + 1) = RSMs(i).RSM(find(trmsk~=0));
    end
    ...
    for i = 1 : size(pcovv, 2)  %check there are different similarities
      ...
      %%%%%%%%%%%% EDIT %%%%%%%%%%%%%%%%%%% 
      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      %%commenté car calcule S avec la fonction svd qui ne peut prendre en argument des input contenant des Nan, ce qui est le cas pour covv. On peut commenter car pas essentiel    
